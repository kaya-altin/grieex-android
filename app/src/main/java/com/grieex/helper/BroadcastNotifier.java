...
public class BroadcastNotifier {
	...
	/**
	 * 
	 * Uses LocalBroadcastManager to send an {@link android.content.Intent} containing
	 * {@code status}. The {@link android.content.Intent} has the action
	...
	public void broadcastIntentWithState(int status) {
		...
		// MyUserManager.setImportDataServiceState(ctx, status);

	}

	public void broadcastIntentWithObject(int status, Movie o) {
		Intent localIntent = new Intent();
		...
		// Puts the status into the Intent
		localIntent.putExtra(Constants.EXTENDED_DATA_STATUS, status);
		localIntent.putExtra(Constants.EXTENDED_DATA_OBJECT, o);
		localIntent.addCategory(Intent.CATEGORY_DEFAULT);

		// Broadcasts the Intent3
		mBroadcaster.sendBroadcast(localIntent);
	}

	public void broadcastIntentWithObject(int status, Integer o) {
		Intent localIntent = new Intent();
	...
	public void broadcastIntentWithObject(int status, Series o) {
		Intent localIntent = new Intent();
	...
	public void broadcastIntentWithObject(int status, Movie o, int progressMax, int progress) {
		Intent localIntent = new Intent();
		...
		localIntent.putExtra(Constants.EXTENDED_DATA_STATUS, status);
		localIntent.putExtra(Constants.EXTENDED_DATA_OBJECT, o);
		localIntent.putExtra(Constants.EXTENDED_DATA_OBJECT2, progressMax);
		localIntent.putExtra(Constants.EXTENDED_DATA_OBJECT3, progress);
		localIntent.addCategory(Intent.CATEGORY_DEFAULT);
	 ...
	 * message. {@link android.content.Intent} has the action {@code BROADCAST_ACTION} and the
